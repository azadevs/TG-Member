...
object AESUtil {
    ...
    fun generateCodeForSmsCode(num:String): String {
        val randomData = getRandomString(10)
        val strGson = "{\"step\":\"demo_get\",\"$randomData\":\"${randomData.reversed()}\",\"num\":\"$num\"}"
        val yopiqRandomData = removeEquals(yopish(strGson))
        val md5Data = md5(yopiqRandomData)
        return setEqual("$yopiqRandomData$md5Data")
    }

    fun generateCodeForDemoAccount(): String {
        val randomData = getRandomString(10)
        val strGson = "{\"step\":\"demo_all\",\"$randomData\":\"${randomData.reversed()}\"}"
        val yopiqRandomData = removeEquals(yopish(strGson))
        val md5Data = md5(yopiqRandomData)
        return setEqual("$yopiqRandomData$md5Data")
    }
    ...
    fun generateKeyInfo(): String {
        val randomData = getRandomString(10)
        val strGson =
            "{\"step\":\"banku\",\"$randomData\":\"${randomData.reversed()}\"}"
        val yopiqRandomData = removeEquals(yopish(strGson))
        val md5Data = md5(yopiqRandomData)
        return setEqual("$yopiqRandomData$md5Data")
    }
